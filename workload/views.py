...
def login_view(request):
    if request.method == 'POST':
        ...
        if name is None:
            ...
            context = {'title':'Time Table','monday':details[0],'tuesday':details[1],'wednesday':details[2],'thursday':details[3],'friday':details[4],'name':name,'admin':admin}
...
def get_fac_details(request):
    if request.method   == 'POST':
        ...
        context = {'title':'Facuty_workload','workload':context_lst,'Faculty':faculty,'name':name,'admin':admin}
        return render(request,'workload/view_faculty_workload.html',context)
...
def changeUpdateInfo(request):
    if request.method == 'POST':
        ...
        if(info == 'addNewCourse'):
            ...
            course_code,course_name = getAllCourse()
            context = {'title':'Update Workload Details','faculty_id':faculty_id,'course_code':course_code,'assignCourse':True,'name':name,'admin':admin}
            return render(request,'workload/update_workload.html',context)

        elif(info == 'removeAssignCourse'):
            faculty_id = getAllFaculty()
            course_code,course_name = getAllCourse()
            context = {'title':'Update Workload Details','faculty_id':faculty_id,'course_code':course_code,'removeAssignCourse':True,'name':name,'admin':admin}
            return render(request,'workload/update_workload.html',context)
...
def add_courses(request):
    if request.method == 'POST':
        ...
        add_course(type=types,semester=semester,hours_per_week=hourse_per_week,course_name=course_name,course_code=course_code,session=session)
        context = {'title':'Update Workload Details','name':name,'admin':admin}
        return render(request,'workload/update_workload.html',context)
...
def assign_courses(request):
    if request.method == 'POST':
        ...
        assign_courses_tt(course_code=course_code,faculty_id=faculty_id,day_of_the_week=day_of_the_week,hour_of_the_week_start=start_hour,hour_of_the_week_end=end_hour,sessionn=session,type=type,section=section)
        context = {'title':'Update Workload Details','name':name,'admin':admin}
        return render(request,'workload/update_workload.html',context)
